...
# For poisson model we can see a little bit fan shape in the first plot, 
# and residuals also deviates from the pointwise confidence envelope in the QQ-plot.
...
# 
# - Information on correlation is stored in the LV loadings \theta_j, 
...
# the environmental conditions driving species abundances at sites
# (as they are not included in the model), 
...
#   by adding latent variable loadings \theta_j to the ordination of sites.
...
#------------------------------------------------------------------
# Assessing the variation explained
#------------------------------------------------------------------
# The variation explained can be assessed by comparing the traces of 
# the residual covariance matrix.
# So here we calculate a pseudo R2 value for the model with env covariates,
# vs model without env covariates:
tr0 <- getResidualCov(fitnb1)$trace
trx <- getResidualCov(fitx1)$trace
1 - trx/tr0
# Based on these, environmental covariates explained 90% of 
